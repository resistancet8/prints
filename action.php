...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM artists";
	$run_query = mysqli_query($con,$category_query) or die(mysqli_error($con));
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h4>Artists</h4></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='artist' artist_id='$artist_id'>$artist_name</a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["artist_id"];
		$sql = "SELECT * FROM products WHERE product_art = '$id'";
	} else {
