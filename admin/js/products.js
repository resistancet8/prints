$(document).ready(function(){
		...
		$.ajax({
			...
			success : function(response){
				...
				if (resp.status == 202) {
					...
					var catSelectHTML = '<option value="">Select Artist</option>';
					
					$.each(resp.message.artists, function(index, value){

						catSelectHTML += '<option value="'+ value.artist_id +'">'+ value.artist_name +'</option>';
	...
	$(document.body).on('click', '.edit-product', function(){
		...
		$("select[name='e_artist_id']").val(product.artist_id);
